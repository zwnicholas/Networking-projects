...
#include <stdlib.h>
#include "queue.h"
// Go-Back-N, malfunctioning, with queue, 11/25/16
...
#define N 8 // N is the window size
...
/* called from layer 5, passed the data to be sent to other side 
   ...
   the network
   If the window is full and A gets a call from above, enqueue the
   message. If space is available in the window, dequeue as many
   enqueued messages as you can until the window is full, and then
   enqueue the message from above.
*/
  ...
  struct msg message;
{
   ...
   if (nextseqnum<base+N) { // if the window is not full
      ...
      int bufIndex = 0;
      if (!isEmpty()) {
         printf("there are queued messages. Enqueuing new message\n");
         enqueue(message.data); // if there are queued messages, enqueue current message
      
         while ((!isEmpty()) && (nextseqnum<base+N)) { // while there are queued 
         // messages and the window is not full, prepare packets for them and send them out
            i = 0;
            sum = 0;
            bufIndex = 0;
            dequeue(pmsg);
            ...
            printf("A_output: Dequeuing, sending pkt %d\n", nextseqnum);
            newpkt.seqnum = nextseqnum;
            newpkt.acknum = -99; // placeholder
            newpkt.checksum = sum + newpkt.seqnum + newpkt.acknum;
            Abuffer[bufIndex].seqnum = nextseqnum;
            Abuffer[bufIndex].acknum = -99;
            Abuffer[bufIndex].checksum = newpkt.checksum;
            for (i = 0; i < 20; i++) {
               newpkt.payload[i] = (*pmsg)[i];
               Abuffer[bufIndex].payload[i] = (*pmsg)[i];
            }
            tolayer3(0, newpkt);
            if (base == nextseqnum) {
               starttimer(0, 15.0);
      ...
      printf("A_output:Queue empty. sending pkt %d\n", nextseqnum);
      for (i = 0; i < 20; i++) {
         sum += (int)message.data[i];
      }
      bufIndex = nextseqnum % 50; // guess. most likely wrong approach
      ...
      for (i = 0; i < 20; i++) {
         ...
         Abuffer[bufIndex].payload[i] = message.data[i];
      }
      tolayer3(0,newpkt);
      if (base == nextseqnum) {
         starttimer(0, 15.0);
      ...
      if (enqueue(message.data) == FALSE) {
         printf("Queue is full. Refusing data\n");
      } 
      else if (enqueue(message.data) == TRUE) {
  ...
  /* 
   newpkt.seqnum = nextseqnum;//(A_bufpkt.seqnum == 1) ? 2 : 1 ;
  ...
  struct pkt packet;
{
   if ((packet.seqnum + packet.acknum == packet.checksum) && (packet.seqnum > 0) && (packet.acknum > 0)){
      ...
      printf("A_input: checksum mismatch! received ACK/NAK %d\n", packet.acknum);
      return;
   }
} 

/* called when A's timer goes off */
// bug with seq # = 0 has to do with the packets being resent by the
// timer interrupt. It has something to do with how the outgoing packets
// are buffered by the sender in Abuffer!
void A_timerinterrupt()
{
   int i = 0;
   printf("Timeout, resending packets %d through %d\n", base, nextseqnum-1);
   printf("Actual packets sent in timerInterrupt:\n\t");
   starttimer(0, 15);
   for (i=0; i<nextseqnum-base; ++i) {
      tolayer3(0,Abuffer[base+i]);
      printf("%d ", Abuffer[base+i].seqnum);
   }
  ...
  struct pkt packet;
{
   ...
   if ((checksum == packet.checksum) && (packet.seqnum == expected_seqnum)) {
     ...
     // should I set packet.payload to NULL after successfully receiving the payload?
      ...
      packet.acknum = 0 - expected_seqnum; 
      ...
      packet.acknum = expected_seqnum - 1;
